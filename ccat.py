...
dict_for_drawing_spanning_tree_plot = {}
# creds harvester
...
for filename in filenames[0]:
    if 'DS_Store' not in filename:
        ...
        result_dict.update(checks.parameters.check(global_params))

        # Adding device name to dictionary
        dict_for_drawing_plot.update({config_name: {}})
        dict_for_drawing_spanning_tree_plot.update({config_name: checks.parameters.spanning_tree(global_params)})
...
if args.args.span != 0:
    graph.draw_spanning_tree(dict_for_drawing_spanning_tree_plot, args.args.span)
