...
def global_parse(config):
    ...
    for line in config:
        ...
        if 'Model Number' in line:
            iface_global['model'] = line[line.find(':')+2:]
        if 'System Serial Number' in line:
            iface_global['serial'] = str(line)[str(line).find(':')+2:]
        if 'Base Ethernet MAC Address' in line:
            iface_global['mac'] = str(line)[str(line).find(':')+2:]
        if 'Cisco IOS' and 'Software' in line:
            iface_global['os'] = str(line)[:str(line).lower().find('version')].split()[1]
            iface_global['ios'] = str(line)[str(line).lower().find('version'):].split()[1]
        if 'This bridge is the root' in line:
            iface_global['spanning-tree'] = "1"
        if 'Cost' in line:
            #add cost only once in dictionary only for VLAN0001
            if 'spanning-tree' not in iface_global.keys():
                iface_global['spanning-tree'] = str(line).split()[1]
...
def _interfaceParse___iface_attributes(config, check_disabled):
    ...
    if iface_list:
        ...
        for option in iface_list[::-1]:
            if_vlan = False
            if 'switchport' in option or 'trunk' in option:
                try_vlan = ''.join(c for c in option if c.isdigit() or c == ' ' or c == ',' or c == '-' )
                if_vlan = True
            ...
            if if_vlan:
                try:
                    ...
                    vlan_add = vlan_num.parseString(try_vlan).asList()
